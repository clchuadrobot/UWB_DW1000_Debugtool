...
int main(void)
{
    ...
    static uint8_t buf[200] = {0};
		...
		uint8_t* buffb[65];
		u8 buff_positiontousb[30] = ""; //Best position to USB
		char position_x[20];
		char position_y[20];
		char position_z[20];
        ...
        if (count >= 0xFFFE) 
				{
					...
					memset(buff_positiontousb,0,sizeof(buff_positiontousb));
					LED_OFF(eLED_0);
					...
					sprintf(position_x, "S%f,", report.x);
					sprintf(position_y, "%f,", report.y);
					sprintf(position_z, "%fE", report.z);
					//sprintf(position_z, "%f\n", sumerror);
					
					
					strcat(buff_positiontousb,position_x);
					strcat(buff_positiontousb,position_y);
					strcat(buff_positiontousb,position_z);
					
					//USB_TxWrite(buff_positiontousb, sizeof(buff_positiontousb));
					Uart1SendStr(buff_positiontousb);
